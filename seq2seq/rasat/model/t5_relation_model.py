...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        def relative_attention_logits(query, key, relation):
            ...
            r_t = relation.transpose(-2, -1)[:,:n_q,:,:n_q]
        ...
        def relative_attention_values(weight, value, relation):
            ...
            # print("wv_matmul shape: ", wv_matmul.shape)
            ...
            # = [batch, num queries, heads, depth]
            w_tr_matmul = torch.matmul(w_t, relation[:,:n_q,:n_q,:])
            # print("w_tr_matmul shape: ", w_tr_matmul.shape)

            # w_tr_matmul_t is [batch, heads, num queries, depth]
            w_tr_matmul_t = w_tr_matmul.permute(0, 2, 1, 3)
            # print("w_tr_matmul_t shape: ", w_tr_matmul_t.shape)
        ...
        #=============================================以下计算关系部分=====================================================================
            ...
            # 1. 首先获取关系矩阵
            relation_k_states = relation_k_emb  # [batch, num queries, num queries, dim ]
            relation_v_states = relation_v_emb  # [batch, num queries, num queries, dim ]
            # relation_k_states = self.relation_k_emb[relations.long()]  # [batch, num queries, num queries, dim ]
            # relation_v_states = self.relation_v_emb[relations.long()]  # [batch, num queries, num queries, dim ]
        
            # 2. 接下来计算关系矩阵的积
            scores = relative_attention_logits(query_states, key_states, relation_k_states)  # [batch, heads, num queries, num kvs]
        ...
        if relation_v_emb is None:
            attn_output = unshape(torch.matmul(attn_weights, value_states))  # (batch_size, seq_length, dim)
        else:
            attn_output = unshape(relative_attention_values(attn_weights, value_states, relation_v_states))
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.SelfAttention(
            ...
            relation_k_emb=relation_k_emb,
            relation_v_emb=relation_v_emb,
            mask=attention_mask,
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, relation_k_emb=None, relation_v_emb=None):
        ...
        self.relation_k_emb = relation_k_emb
        self.relation_v_emb = relation_v_emb
        # self.relation_k_emb = torch.rand(self.relation_kinds+1, config.d_kv, requires_grad=True)
        # self.relation_v_emb = torch.rand(self.relation_kinds+1, config.d_kv, requires_grad=True)
        # self.relation_k_emb = torch.rand(self.relation_kinds+1, config.d_kv, requires_grad=False)
        # self.relation_v_emb = torch.rand(self.relation_kinds+1, config.d_kv, requires_grad=False)
    ...
    def forward(
        ...
        # ================================Modify===========================================
        # print("inputs_embed.shape before hidden_states: ", inputs_embeds.shape)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.relation_kinds = config.num_relations
        # self.relation_kinds = None
        if self.relation_kinds is not None:
            self.relation_k_emb = nn.Embedding(self.relation_kinds+1, config.d_kv, padding_idx=0)
            self.relation_v_emb = nn.Embedding(self.relation_kinds+1, config.d_kv, padding_idx=0)
            # print("self.relation_k_emb(1): ", self.relation_k_emb)
        else:
            self.relation_k_emb = None
            self.relation_v_emb = None      

        encoder_config = copy.deepcopy(config)
        encoder_config.is_decoder = False
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, self.relation_k_emb, self.relation_v_emb)
